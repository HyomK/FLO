    ...
    fun signUp(user : User){
        ...
        authService.signUp(user).enqueue(object: Callback<AuthResponse>{
            override fun onResponse(call: Call<AuthResponse>, response: Response<AuthResponse>) {
                ...
                when(resp.code){
                    1000->signUpView.onSignUpSuccess(resp.result!!.userIdx, resp.result.jwt)
                    else->{
    ...
    fun login(user : User){
        ...
        authService.login(user ).enqueue(object: Callback<AuthResponse>{
            override fun onResponse(call: Call<AuthResponse>, response: Response<AuthResponse>) {
                ...
                Log.d("LOGINACT/RESPONSE-FLO",resp.toString())
                when(resp.code){
                    1000->{
            ...
            override fun onFailure(call: Call<AuthResponse>, t: Throwable) {
                Log.d("LOGINACT/API-RESPONSE",t.message.toString())
                loginView.onLoginFailure(400,"네트워크 오류가 발생했습니다.")
            }
        })
        Log.d("LOGINACTACT/ASYNC","logined")
